...
import type { MatchHistoryEntry } from "../queries/player-match-history";
...
function ownerOnSideA(overrides: Partial<MatchHistoryEntry> = {}) {
  return {
    id: "match-1",
    status: MatchStatus.PENDING_CONFIRMATION,
    recordedAt: new Date("2026-08-11T17:32:00.000Z"),
    participants: [
      { ...owner, side: MatchSide.A },
      { ...partner, side: MatchSide.A },
      { ...opponentOne, side: MatchSide.B },
      { ...opponentTwo, side: MatchSide.B },
    ],
    sets: [
...
function ownerOnSideB(overrides: Partial<MatchHistoryEntry> = {}) {
  return ownerOnSideA({
    participants: [
      { ...opponentOne, side: MatchSide.A },
      { ...opponentTwo, side: MatchSide.A },
      { ...owner, side: MatchSide.B },
      { ...partner, side: MatchSide.B },
    ],
    ...overrides,
  });
}
...
describe("PlayerMatchHistory", () => {
  ...
  it("should not show points anywhere", () => {
    render(
      <PlayerMatchHistory
        playerId={OWNER_ID}
        matches={[ownerOnSideA({ status: MatchStatus.SCORED })]}
      />,
    );
