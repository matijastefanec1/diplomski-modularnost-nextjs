...
import type { MatchDetails, MatchStatus } from "@/src/application/match";
...
describe("MatchDecisionPage", () => {
  it("shows the recorded time, the status and the sets from team A's side", () => {
    ...
    expect(screen.getByTestId("match-sets")).toHaveTextContent("6:4, 3:6, 7:5");
  });
  ...
  it("shows an expired match without the deadline row", () => {
    render(
      <MatchDecisionPage
        match={matchDetails({ status: "EXPIRED" })}
        now={AFTER_DEADLINE}
        viewerId={null}
      />,
    );
  ...
  it("should not show points anywhere", () => {
    render(
      <MatchDecisionPage
        match={matchDetails({ status: "SCORED" })}
        now={DURING_WINDOW}
  ...
  it.each<MatchStatus>(["SCORED", "DISPUTED", "EXPIRED"])(
