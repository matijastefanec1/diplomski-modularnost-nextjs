...
export type RecordMatchParticipants = {
  partnerId: string;
  opponentOneId: string;
  opponentTwoId: string;
};

export type ValidatedRecordMatch = RecordMatchParticipants & {
  sets: MatchSets;
};
...
export function toMatchResultIssue(violation: MatchResultViolation): {
  path: string;
  message: string;
} {
...
export function recordMatchSchema(reporterId: string) {
  return z
    ...
    .superRefine((value, ctx) => {
      for (const field of duplicateFields(value, reporterId)) {
        ctx.addIssue({
          code: "custom",
